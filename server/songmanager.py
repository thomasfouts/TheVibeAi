...
class SongManager:
    ...
    def get_audio_features(self, track_ids):
        try:
            ...
            return {track['uri']: track for track in features if track is not None}
        except Exception as e:
    ...
    def select_best_tracks(self, artist_ids, current_song=None):
        ...
        for artist_id in artist_ids:
            ...
            track_lists.append((track_ids, track_vectors))
        ...
        m = len(track_lists[0][1])
        dp = [[float('inf')] * len(track_lists[i][1]) for i in range(n)]
        choice = [[-1] * len(track_lists[i][1]) for i in range(n)]

        for j in range(len(track_lists[0][1])):
            dp[0][j] = 0

        for i in range(1, n):
            for j in range(len(track_lists[i][1])):
                for k in range(len(track_lists[i - 1][1])):
                    cost = self.song_difference(track_lists[i][1][j], track_lists[i - 1][1][k])
        ...
        min_cost = float('inf')
        last_choice = -1
        for j in range(len(track_lists[n - 1][1])):
