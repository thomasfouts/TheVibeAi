...
class Vibe:
    def __init__(self, sp_utils = None, sp = None, access_token = None, artist_graph = None, song_manager = None):
        ...
        if(self.artist_graph == None):
            ...
            self.artist_graph = ArtistGraph('data/nodes.js', 'data/base_edge_weights.json', 'data/edges.js', access_token=access_token)
        
        if(self.song_manager == None):
            print('Creating song manager')
            self.song_manager = SongManager(artist_top_tracks_file='data/artisttoptracks.js', access_token = access_token)
